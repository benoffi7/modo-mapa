...
export function useLocalTrending(): UseLocalTrendingResult {
  ...
  // Consume the shared business singleton directly — no local projection.
  ...
  const { filtered, radiusKm } = useMemo(() => {
    ...
    for (const radius of LOCAL_TRENDING_RADII) {
      const result = data.businesses.filter((biz) => {
        const fullBiz = businessMap.get(biz.businessId);
        if (!fullBiz) return false;
        return distanceKm(location.lat, location.lng, fullBiz.lat, fullBiz.lng) <= radius;
      });
  ...
  }, [data, location.lat, location.lng, businessMap]);
