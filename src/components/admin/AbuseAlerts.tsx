...
import {
  computeKpis, getDateThreshold, exportToCsv,
  ALL_TYPES, PAGE_SIZE, DATE_PRESETS, STATUS_OPTIONS,
} from './alerts/alertsHelpers';
import type { AbuseType, SortField, SortDir, DatePreset, StatusFilter } from './alerts/alertsHelpers';
...
export default function AbuseAlerts() {
  ...
  const effectiveLogs = useMemo(() => {
    ...
    return logs.map((log) => { const u = localUpdates.get(log.id); return u ? { ...log, ...u } : log; });
  }, [logs, localUpdates]);
  ...
  const handleReview = async (logId: string, e: React.MouseEvent) => {
    ...
    try { await reviewAbuseLog(logId); setLocalUpdates((p) => new Map(p).set(logId, { ...p.get(logId), reviewed: true })); }
  ...
  const handleDismiss = async (logId: string, e: React.MouseEvent) => {
    e.stopPropagation(); setActionInProgress(logId);
    try { await dismissAbuseLog(logId); setLocalUpdates((p) => new Map(p).set(logId, { ...p.get(logId), dismissed: true })); }
  ...
  const clearFilters = () => { setTypeFilter('all'); setCollectionFilter(''); setUserSearch(''); setDatePreset('all'); setStatusFilter('pending'); setVisibleCount(PAGE_SIZE); };
  const hasActiveFilters = typeFilter !== 'all' || collectionFilter !== '' || userSearch !== '' || datePreset !== 'all' || statusFilter !== 'pending';
