 ...
 * `scripts/pre-staging-check.sh` solo flagea reads sincronicos raw del SDK
 * (las funciones get-doc/get-docs unicas), no subscriptions, por lo cual
 * NO se requiere el marker `// perf-instrument-ok` (que tiene semantica
 * especifica: suprimir el flag de read raw envuelto en un `Promise.all` ya
 * medido). Si en el futuro se desea telemetria de tiempo-hasta-primer-snapshot,
 * usar `trackEvent('admin_abuse_subscribe', { ms })` con un timer manual
 * (deferido — entrada futura en `docs/reports/tech-debt.md`).
 */
