...
| **BusinessScopeContext** (#306) | `src/context/BusinessScopeContext.tsx` — contexto React local al subarbol de `BusinessSheetContent`. Expone `{ businessId: string, businessName: string, location: { lat: number, lng: number } }`. `useMemo` con dependencias primitivas (id, name, lat, lng) para evitar re-renders cuando el padre reconstruye el objeto scope en cada render. `useBusinessScope()` tira `Error` descriptivo fuera del provider — permite detectar uso incorrecto en desarrollo. Componentes que requieren `BusinessScopeProvider` deben documentarlo en JSDoc. El contexto expone solo **datos**, no callbacks — los handlers siguen pasando como props para mantener la boundary de estado en `BusinessSheetContent`. |
...
| **Marker `// perf-instrument-ok`** | Cuando un `getDocs(...)`/`getDoc(...)` raw esta dentro de un `Promise.all` ya envuelto por `measureAsync`, agregar el comentario `// perf-instrument-ok` en la misma linea o hasta 2 lineas arriba. El check `pre-staging-check.sh` lo respeta. Documenta la decision deliberada de medir en agregado en vez de por-batch. NO usar el marker para evadir instrumentacion sin justificacion. |
...
| **Logger centralizado** | `src/utils/logger.ts` — `logger.error()`, `.warn()`, `.log()`. En DEV: console. En PROD: errors a Sentry, warn/log silenciados. Nunca usar `console.*` directamente. **CRITICO:** `logger.error` NUNCA debe envolverse en `if (import.meta.env.DEV)` — rompe la captura Sentry en prod (#294). |
...
| **Offline action types** | Union discriminada `OfflineActionType` con 18 tipos. Original 9 + 6 de listas (`list_create`, `list_update`, `list_toggle_public`, `list_delete`, `list_item_add`, `list_item_remove`) + 3 agregados en #323 (`comment_edit`, `comment_delete`, `rating_criteria_upsert`). Payloads tipados por tipo de accion en `types/offline.ts`. `OfflineAction` incluye campo opcional `listId` para operaciones de listas (ademas del `businessId` requerido por backward compat). `list_delete` y operaciones destructivas (delete account, clean anon data) se bloquean offline (cascade/irreversibles inseguros) — boton deshabilitado en UI con tooltip "Requiere conexión". Replay: `syncEngine.executeAction` switch con dynamic `import('./service')` para cold-start. #323 cubrió ~17 superficies UI con `withOfflineSupport` o gate offline. |
