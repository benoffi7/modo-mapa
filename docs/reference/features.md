...
| **Alertas** | Logs de abuso (rate limit excedido, contenido flaggeado, top writers). KPI cards resumen (alertas hoy con tendencia vs ayer, tipo más frecuente, usuario más activo, total). Filtros por rango de fechas (presets: Hoy, Última semana, Último mes). Export CSV de alertas filtradas |
