...
- **MapErrorBoundary** (#304): Error Boundary de clase React en `components/search/MapErrorBoundary.tsx`. Captura cualquier throw del subarbol de Google Maps y auto-cambia a list view via callback `onFallback`. Si `VITE_GOOGLE_MAPS_API_KEY` no esta configurado, fuerza list view desde el inicio. Boton "Reintentar" permite recovery sin recarga de pagina. Analytics: `EVT_MAP_LOAD_FAILED` en `constants/analyticsEvents/system.ts`
...
- **Pantalla full** (`BusinessDetailScreen` en `/comercio/:id`): chip tabs sticky (Criterios / Precio / Tags / Foto / Opiniones). Header sticky + chip bar sticky (`top: headerHeight` medido con ResizeObserver+useLayoutEffect). Contenido de tab inactivo oculto con `display: none`. Botón "Volver" navega `-1` o a `/` si es deep link directo. Analytics: `business_detail_opened`, `business_detail_tab_changed`, `business_detail_cta_clicked`, `sub_tab_switched`. Deep link: `/comercio/:id?tab=criterios|precio|tags|foto|opiniones`. Layout responsive: `maxWidth: { sm: 640, md: 720 }` centrado sobre `bgcolor: background.default`. OG meta tags (`og:title`, `og:description`, `og:url`, `og:type`) seteados client-side en `BusinessDetailPage`; `document.title` restaurado al desmontar. Chip bar con gradiente de overflow (ResizeObserver + scroll listener detectan si hay más chips ocultos a la derecha). A11y: `role="tablist"` en el bar, `role="tab"` + `aria-selected` + `tabIndex` roving + teclado (ArrowLeft/Right/Home/End) en cada chip.
...
## Navegacion principal (TabBar + tabs)

- **BottomNavigation de 5 tabs** (`src/components/layout/TabBar.tsx`): **Inicio / Social / Buscar / Listas / Perfil**. Altura fija `TAB_BAR_HEIGHT = 64px`. Badges de error: tab Social (recomendaciones no leidas via `useUnreadRecommendations`) y tab Perfil (notificaciones via `useNotifications`). Cada tab lazy-loaded via `React.lazy()` + `Suspense` con `TabLoader` (`src/components/ui/TabLoader.tsx`) como fallback de skeleton al cambiar de tab (reduce main chunk ~25%).
- **SearchFab** (`src/components/layout/SearchFab.tsx`): FAB central elevado sobre el TabBar, activo cuando el tab Buscar esta seleccionado (prop `active`). Abre la busqueda de comercios.
- **TabShell** (`src/components/layout/TabShell.tsx`): orquesta los 5 tabs. Rutea cada tab a su Screen: `inicio` → `HomeScreen`, `social` → `SocialScreen`, `buscar` → `SearchScreen`, `listas` → `ListsScreen`, `perfil` → `ProfileScreen`. Tabpanels implementados via `display: none` (no unmount) para preservar estado al cambiar de tab. Usa `useNavLayout` para calcular posicion bottom/left del contenido segun orientacion del dispositivo. Badges pasados como props a `TabBar` (`notificationBadge`, `recommendationBadge`).
- Todas las screens lazy-loaded con `React.lazy()` + `Suspense`. `TabLoader` es el spinner/skeleton de fallback al cambiar de tab.

### Tab Inicio → HomeScreen

- Header con avatar (seleccion de avatar persiste en Firestore `users/{uid}.avatarId`), nombre, boton editar nombre, badge tipo de cuenta (temporal/email+verificado), botones "Crear cuenta" / "Ya tengo cuenta" (solo anonimos). Badge en icono de Comentarios con cantidad de respuestas no leidas (unread `comment_reply` count). **Onboarding gamificado (primeros pasos)**: card "Primeros pasos" con 5 tareas (calificar, comentar, favoritear, agregar tag, explorar ranking). Progress bar, checks verdes al completar. Colapsable con click en header (estado persiste en localStorage). Dismiss permanente via localStorage. Toast de celebracion al completar todas. Solo para usuarios autenticados (no anonimos). **Nudge de verificacion** (#157): card para usuarios con email no verificado con botones "Re-enviar email" y "Ya verifique". Dismisseable por sesion, reaparece en sesiones futuras si no verifico.
- **Sorprendeme**: boton que selecciona un comercio al azar no visitado. Prefiere cercanos (radio 5km) si GPS activo. Toast con nombre del comercio. Fallback a random si todos visitados.
...
- **Actividad** (#129): feed de actividad de usuarios seguidos (ratings, comentarios, favoritos). Paginado, pull-to-refresh. Items expiran a 30 dias. Ver seccion "Seguir usuarios". Badge en el tab Social (`TabBar`) con conteo de no leidas.
- **Recomendaciones recibidas** (#135): seccion Recomendaciones en el tab Social (`ReceivedRecommendations`). Lista paginada (20 por pagina). Items muestran avatar con inicial, nombre del remitente, nombre del comercio, mensaje entre comillas, fecha relativa. Indicador visual de no leido (`action.hover` background). Click navega al comercio y marca como leida. "Marcar todas como leidas" al abrir la seccion. el tab Social (`TabBar`) muestra badge `color="error"` con conteo de recomendaciones no leidas. Lazy-loaded en el tab Social. Ver seccion "Recomendaciones entre usuarios".
...
- **Comentarios**: lista mejorada con skeleton loader, empty state motivacional, preview enriquecido (fecha relativa, indicador editado, likes, replies), busqueda de texto (con `useDeferredValue` + auto-load-all), ordenamiento (recientes/antiguos/mas likes via `ToggleButtonGroup`), filtro por comercio (Autocomplete), edicion inline, stats resumen colapsable (total, likes, promedio, mas popular), swipe actions en mobile (deslizar para editar/eliminar), undo delete con `useUndoDelete` hook. Envuelto en `PaginatedListShell` para loading/error/empty states consistentes. Pull-to-refresh gesture. Blue dot indicator en comentarios con respuestas no leidas; mark-as-read al hacer click. `CommentItem` extraido y memoizado (`React.memo`). Virtualizacion condicional via `useVirtualizedList` hook con `@tanstack/react-virtual` cuando hay >= 20 items. Filtros extraidos a `useCommentsListFilters` hook (#195). En la lista de comentarios del perfil (tab Perfil), las preguntas muestran badge "Pregunta" en la seccion Comentarios.
...
- **Logros**: ver seccion "Nota: Badges vs Achievements".
- **Notificaciones**: ver seccion "Notificaciones in-app". Badge en el tab Perfil con conteo de notificaciones no leidas. La campana vive en la barra de busqueda (tab Buscar), no en ningun menu.

### Ajustes (dentro de tab Perfil → SettingsPanel)

- **Configuracion (cuenta)**: seccion Cuenta (primera, extraida a `AccountSection` componente en #195): anonimos ven crear cuenta/login + "Empezar de cero", usuarios email ven email + badge del tab Perfil y en SettingsPanel (verificacion) + re-enviar verificacion (con cooldown via `useVerificationCooldown` hook) + cambiar contrasena + eliminar cuenta + cerrar sesion con confirmacion. **Ubicacion** (#154): campo opcional de localidad (ciudad/barrio) con Google Places Autocomplete, usado como ubicacion por defecto sin GPS. Fallback chain: GPS → localidad → oficina (via `useSortLocation` hook). Afecta mapa, sort por cercania, Sorprendeme y sugerencias. **Apariencia** (#231): toggle de dark mode (id ayuda `modooscuro`, "Modo oscuro") con `useColorMode` hook, ubicado entre las secciones Ubicacion y Privacidad. Toggles de privacidad (perfil publico/privado), notificaciones (master + likes/fotos/rankings/feedback/replies/nuevos seguidores/recomendaciones), y datos de uso (analytics). Defaults todos en false excepto `notifyFollowers: true` y `notifyRecommendations: true`. Optimistic UI con revert on error.
- **Onboarding**: guia de bienvenida y configuracion inicial. Ver seccion "Autenticacion por email/password".
...
- **Ayuda**: seccion colapsable con topics en formato Accordion, agrupados por tab (Inicio, Buscar, Social, Listas, Perfil, Ajustes) con chips como separadores. Contenido definido en el registry `src/components/profile/helpGroups.tsx` (patron HELP_GROUPS, equivalente a `HOME_SECTIONS`); cubre modo offline (#136, #304), rankings con tiers/badges/"Mi zona" (#200), perfil publico de otros usuarios, Q&A con Mejor respuesta (#127), fotos de menu con staleness, digest frequency, onboarding de cuenta (#157), color/icon picker de listas, Compartidas conmigo y Destacadas, recomendaciones entre usuarios (#135), y configuracion de Apariencia/dark mode. `HELP_GROUPS` exporta interfaces `HelpItem` y `HelpGroup` con campos `id`, `icon` (ReactElement MUI), `title`, `description`. `AVATAR_COUNT` dinamico validado contra `AVATAR_OPTIONS.length` al build. Cada `AccordionSummary` tiene `aria-label` explicito. Lazy-loaded via `React.lazy()`. Componente: `HelpSection.tsx` (#311).
- **Agregar comercio**: link externo a Google Forms.
- Footer con version de la app (+ links a Theme Playground y Constants Dashboard en DEV). Accesible desde el footer de ProfileScreen (DEV).
...
- `NotificationsContext` centralizado: instancia unica compartida por todos los consumidores (campana, badge del tab Perfil, etc.)
...
- **Verificacion de email**: no bloqueante. Badge en el tab Perfil y en SettingsPanel. Re-enviar con cooldown 60s. Refresh via `user.reload()`
...
- **Recibir recomendaciones**: seccion Recomendaciones en el tab Social (`ReceivedRecommendations`). Lista paginada (20 por pagina) via `PaginatedListShell`. Items muestran avatar con inicial, nombre del remitente, nombre del comercio, mensaje entre comillas, fecha relativa. Indicador visual de no leido (`action.hover` background). Click navega al comercio y marca como leida. "Marcar todas como leidas" al abrir la seccion
- **Badge**: el tab Social (`TabBar`) muestra badge `color="error"` con conteo de recomendaciones no leidas via `useUnreadRecommendations` hook (`getCountFromServer`)
...
- **Componentes**: `RecommendDialog.tsx` (lazy-loaded en BusinessSheet), `ReceivedRecommendations.tsx` (lazy-loaded en el tab Social)
...
- Accesible desde link en el footer de ProfileScreen (DEV)
...
| **Alertas** | Logs de abuso (rate limit excedido, contenido flaggeado, top writers). KPI cards resumen (alertas hoy con tendencia vs ayer, tipo más frecuente, usuario más activo, total). Filtros organizados en grid 2 columnas: periodo (Hoy/Semana/Mes), estado (Pendientes/Revisadas/Descartadas/Todas), tipo, severidad (Baja/Media/Alta), colección, búsqueda por userId. Acciones: Revisar y Descartar alertas. Detalle inline con total alertas del usuario y badge "Reincidente" (>3 alertas). Export CSV. **Realtime**: `onSnapshot` con toast de nuevas alertas. Badge de pendientes en tab. **Reincidentes**: pestaña con tabla de usuarios con >N alertas (filtros >3, >5, >10), historial expandible por usuario. **Rate Limits** (#327): subtab inspector + reset de `_rateLimits/{key}`. Filtro por User ID (debounced), tabla con categoria/userId/count/resetAt relativo/estado, accion Resetear con dialog `role="alertdialog"` (copy housekeeping cuando la ventana ya expiró). El reset desbloquea al usuario inmediatamente, escribe `abuseLog` con `type: 'config_edit'`. Gating offline via `useConnectivity`. Race multi-admin: `HttpsError 'not-found'` se mapea a toast info + refetch. Mientras el subtab está activo se pausa `useAbuseLogsRealtime` (`enabled=false`) para no consumir reads. KPIs y filtros de abuso quedan ocultos en este subtab. **Rate Limits IP** (#348): cuarto subtab inspector + reset de `_ipRateLimits/{ipHash}_{action}_{date}` (hash-only, nunca IP raw). Filtros por acción y por IP Hash (debounced), tabla con ipHash truncado (Tooltip con hash completo) / acción / fecha / count / estado (Activa/Expirada por `date === hoyUTC`), acción Resetear con dialog `role="alertdialog"`. El acceso pasa por el callable `adminListIpRateLimits` (30/día por admin); el reset (`adminResetIpRateLimit`, 20/día) borra el doc y escribe `abuseLog` `type: 'config_edit'`. Mismo gating offline y mismo patrón de pausa de `useAbuseLogsRealtime` que Rate Limits. Caption aclara que se muestra un hash SHA-256, no la IP real |
...
| **Listas** | Stats globales (total, publicas, privadas, colaborativas, total items, promedio items/lista), top 10 listas por tamano. Toggle listas destacadas (callable getPublicLists/toggleFeaturedList). **Inspeccion + delete inline** (#327): expandir una lista trae sus items (truncados a 50, con texto "Mostrando 50 de N" si la lista tiene mas), cada item muestra el `addedBy` resuelto a displayName via `fetchUserDisplayNames` con chip Owner/Editor segun coincida con `list.ownerId`. Cada item tiene IconButton delete con `aria-label` descriptivo, dialog `role="alertdialog"`, callable `adminDeleteListItem` que decrementa `itemCount` atomicamente y escribe `abuseLog`. Optimistic decrement local de `itemCount` + drop del item en `expandedItems` (sin re-fetch full para evitar flicker). Race multi-admin: `not-found` -> toast info + refetch. Analytics: `admin_list_items_inspected` (1x por listId por sesion via `useRef<Set>`) y `admin_list_item_deleted` |
| **Funcionalidades** | Metricas por funcionalidad: cards con numero de hoy (prominente) + total acumulado. Tendencia vs ayer (flecha). Click expande grafico lineal 30 dias. Features Firestore: ratings, comments, likes, favorites, tags, feedback, check-ins, follows, recomendaciones, nivel de gasto. Features GA4: Sorprendeme, listas, busqueda, compartir, fotos, dark mode, preguntas Q&A. Categoria System (#304/#327) incluye "Errores de mapa" (eventName `map_load_failed` emitido por `MapErrorBoundary`) — pico de errores del SDK de Google Maps (quota/key/blockers) ahora visible. Cache en `config/analyticsCache` con TTL 1h. Graceful degradation si GA4 API falla. Seccion Adopcion: usuarios totales, activos hoy, tasa de actividad |
...
- **Cola offline en IndexedDB**: cuando no hay conexion, las acciones del usuario (ratings, comments, favorites, price levels, tags, listas CRUD) se encolan en IndexedDB (`modo-mapa-offline`) en vez de fallar. Max 50 acciones, TTL 7 dias
...
- **Wrapper pattern**: `withOfflineSupport()` envuelve las llamadas a servicios en los componentes. Los servicios (`ratings.ts`, `comments.ts`, `sharedLists.ts`, etc.) no se modifican. UI optimista existente sigue funcionando. `withOfflineSupport` acepta campo `listId` en `actionMeta` para operaciones de listas (#304)
- **Offline Lists CRUD** (#304): 6 nuevas variantes en `OfflineActionType` — `list_create`, `list_update`, `list_toggle_public`, `list_delete`, `list_item_add`, `list_item_remove`. `list_delete` bloqueado offline intencionalmente (cascade delete inseguro). `createList` acepta `listId` opcional para generar ID client-side via `generateListId()`. Orden FIFO del queue preserva `list_create` antes de `list_item_add`. Boton eliminar lista deshabilitado cuando offline. `NotificationsContext` (`markRead`/`markAllRead`) hace early-return silencioso offline. `EditDisplayNameDialog` y avatar picker bloquean con toast offline
...
- **Seccion Pendientes** (cola de acciones offline): visible solo si hay acciones pendientes; se expone en el tab **Perfil** (`ProfileScreen`, componente `PendingActionsSection.tsx`). Lista con icono por tipo, nombre del comercio, fecha relativa. Boton descartar individual. Boton "Reintentar fallidas" si hay acciones `failed`. Iconos y labels para action types de listas (`FormatListBulletedIcon`, `BookmarkIcon`, `BookmarkRemoveIcon`)
...
- **SpecialsSection**: seccion de cards en HomeScreen que muestra contenido destacado (trending, listas destacadas, logros). Cada card navega a la seccion correspondiente: `trending` navega al tab Listas (Recientes), `featured_list` navega al tab Listas
