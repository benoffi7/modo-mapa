...
- Secciones:
  ...
  - **Ayuda**: seccion colapsable con topics en formato Accordion, agrupados por tab (Inicio, Buscar, Social, Listas, Perfil, Ajustes) con chips como separadores. Contenido definido en el registry `src/components/profile/helpGroups.tsx` (patron HELP_GROUPS, equivalente a `HOME_SECTIONS`); cubre modo offline (#136), rankings con tiers/badges/"Mi zona" (#200), perfil publico de otros usuarios, Q&A con Mejor respuesta (#127), fotos de menu con staleness, digest frequency, onboarding de cuenta (#157), color/icon picker de listas, Compartidas conmigo y Destacadas. Cada `AccordionSummary` tiene `aria-label` explicito. Lazy-loaded via `React.lazy()`. Componente: `HelpSection.tsx`
