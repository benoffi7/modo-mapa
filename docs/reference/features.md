...
- **Seccion Pendientes en SideMenu**: visible solo si hay acciones pendientes. Lista con icono por tipo, nombre del comercio, fecha relativa. Boton descartar individual. Boton "Reintentar fallidas" si hay acciones `failed`. Iconos y labels para action types de listas (`FormatListBulletedIcon`, `BookmarkIcon`, `BookmarkRemoveIcon`)
