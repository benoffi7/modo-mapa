...
| `config` | `counters`, `moderation`, `perfCounters`, `appVersion`, `bootstrap` | counters: totales + daily reads/writes/deletes; moderation: bannedWords; perfCounters: Cloud Function timings (array union per function name, reset daily); appVersion: minVersion (semver string), updatedAt (timestamp) — force-update check (#191); **bootstrap (#322)**: adminAssigned (bool) — flag idempotente que evita re-asignacion del primer admin via `setAdminClaim`. | appVersion: public read, Admin SDK write; bootstrap: admin read, Admin SDK write (escrito por `setAdminClaim` tras asignar claim al primer admin); others: admin read, Functions write |
