...
|-----------|--------|--------|--------|
| `users` | `{userId}` | displayName (1-30, regex `^[A-Za-z0-9À-ÿ_-]([A-Za-z0-9À-ÿ ._-]*[A-Za-z0-9À-ÿ_-])?$`), displayNameLower (== displayName.lower(), bidireccional create+update), avatarId?, createdAt, followersCount (server), followingCount (server) | R/W owner; admin read. Hardening #322: regex condicional sobre `affectedKeys()` para no romper updates de avatar; equality bidireccional en update si se modifica displayName **o** displayNameLower. |
