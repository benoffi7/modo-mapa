...
| `userSettings` | `{userId}` | profilePublic, notificationsEnabled, notifyLikes, notifyPhotos, notifyRankings, notifyFeedback, notifyReplies, notifyFollowers, notifyRecommendations, notificationDigest?, analyticsEnabled, locality?, localityLat?, localityLng?, followedTags? (list<=20), followedTagsUpdatedAt? (timestamp), followedTagsLastSeenAt? (timestamp), updatedAt | Read auth; write owner (`keys().hasOnly`, notifyFollowers/notifyRecommendations validated as bool, notificationDigest as string<=10) |
