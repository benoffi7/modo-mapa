...
|-----------|--------|--------|--------|
| `users` | `{userId}` | displayName (1-30, regex `^[A-Za-z0-9À-ÿ_-]([A-Za-z0-9À-ÿ ._-]*[A-Za-z0-9À-ÿ_-])?$`), displayNameLower (== displayName.lower(), bidireccional create+update), avatarId? (string, `.size()<=50` create+update), createdAt, followersCount (server), followingCount (server) | R/W owner; admin read. Hardening #322: regex condicional sobre `affectedKeys()` para no romper updates de avatar; equality bidireccional en update si se modifica displayName **o** displayNameLower. **#341:** create acepta dos formas — (1) solo-avatar (`avatarId`+`createdAt`, displayName/displayNameLower ausentes) o (2) display completo (displayName+displayNameLower sincronizados, `avatarId?`); nunca `displayNameLower` sin `displayName` (cierra hueco B5). `updateUserAvatar` hace `setDoc` solo-avatar si el doc no existe (paridad con `updateUserDisplayName`). |
...
| `commentLikes` | `{userId}__{commentId}` | userId, commentId, businessId, createdAt | Read auth; create/delete owner (`keys().hasOnly` incluye businessId, `isValidBusinessId`, createdAt==request.time). Query directa por `(userId, businessId)` — índice compuesto `commentLikes(userId, businessId)` (#343, guard #302 R3) |
| `userTags` | `{userId}__{businessId}__{tagId}` | userId, businessId, tagId, createdAt | Read auth; create/delete owner |
| `customTags` | client-side o auto-generated | userId, businessId, label (1-30), createdAt | Read auth; create/update/delete owner |
...
| `config` | `counters`, `moderation`, `perfCounters`, `appVersion`, `bootstrap` | counters: totales + daily reads/writes/deletes; moderation: bannedWords; perfCounters: Cloud Function timings — array por funcion con cap **MAX_SAMPLES_PER_FUNCTION=2000** (#325, bajado desde 5000) + truncate-in-handler (`existing.slice(-(cap-1))`) para no perder samples nuevos al alcanzar el limite. Tamano por field con cap=2000: ~30 KB; con ~30 funciones tracked queda ~900 KB, bajo el limite de 1 MB de Firestore. Reset diario por `dailyMetrics` que lee, calcula p50/p95 y resetea el array. appVersion: minVersion (semver string), updatedAt (timestamp) — force-update check (#191); **bootstrap (#322)**: adminAssigned (bool) — flag idempotente que evita re-asignacion del primer admin via `setAdminClaim`. | appVersion: public read, Admin SDK write; bootstrap: admin read, Admin SDK write (escrito por `setAdminClaim` tras asignar claim al primer admin); others: admin read, Functions write |
...
| `_rateLimits` | `backup_{userId}`, `perf_{userId}`, `delete_{userId}`, `clean_{userId}`, `editors_invite_{userId}`, `editors_remove_{userId}`, `featured_lists_{userId}`, `checkin_create_suspended_{userId}` | count, resetAt, userId; **suspended (#322)**: `suspendedUntil` (number, ms epoch), reason (string) | No client access; Functions write (admin SDK). Campo `userId` permite cleanup en account deletion via `deleteAllUserData`. Usado por backups (5/min), perfMetrics (5/dia), deleteUserAccount (1/min), cleanAnonymousData (1/min), inviteListEditor (10/dia), removeListEditor (10/dia), getFeaturedLists (20/dia, #322), `checkin_create_suspended_{userId}` (#322): flag de suspension 24h escrito por `onCheckInDeleted` cuando un usuario borra ≥20 check-ins; `onCheckInCreated` rechaza creates mientras `suspendedUntil > Date.now()`. |
