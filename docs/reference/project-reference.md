...
- **Tests**: 216+ test files (160 frontend + 56 functions), 1476 frontend + 510 functions tests (#322 sumó +32 tests en functions: claims/featuredLists/inviteListEditor/removeListEditor/cleanAnonymousData/checkins). Incluye: utils, services, hooks, contexts, auth, onboarding, follows, activity feed, triggers, aggregates, helpers, readCache, contrast, useRatingPrompt, useCommentEdit, useVerificationCooldown, sharedLists, BusinessScopeContext, businessMap, MapErrorBoundary, helpGroups, forceUpdate utils, fanOut batch, useForceUpdate concurrency+debounce guards. Politica: >=80% cobertura para features nuevas. Ver [tests.md](tests.md)
