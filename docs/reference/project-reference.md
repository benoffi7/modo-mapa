...
- **Admin** (`/admin`): 16 tabs — overview, actividad, feedback (con responder/resolver/crear issue GitHub), tendencias, usuarios, Firebase usage, alertas (con revisar/descartar, filtro por estado, badge reincidente, **subtab Rate Limits** #327: inspector + reset de `_rateLimits`; **subtab Rate Limits IP** #348: inspector + reset de `_ipRateLimits` hash-only vía callable, y el bloqueo por IP emite `abuseLog` `type: 'ip_rate_limit'`), backups, fotos, performance (descompuesto en subcomponentes), **features** (metricas por funcionalidad con graficos 30 dias; categoria System cuenta `map_load_failed` desde #327), listas (con **delete inline** de items #327: dialog confirmacion + chip Owner/Editor + truncado a 50 con helper "Mostrando 50 de N"), notificaciones, **social** (metricas sociales: follows, activity feed, recomendaciones), **especiales** (CRUD tarjetas especiales para Inicio), **logros** (CRUD definiciones de logros)
...
- **businessMap singleton** (#302, expandido en #324): `src/utils/businessMap.ts` — `getBusinessMap()` devuelve `Map<string, Business>` memoizado a nivel modulo. `getBusinessById(id)` para lookups O(1). #324 migró 13 callsites adicionales (7 hooks: useDeepLinks/useNavigateToBusiness/useBusinessById/useRatingPrompt/useSuggestions/useVisitHistory/useCommentsListFilters; 4 components: FavoritesList/ListDetailScreen/ReceivedRecommendations/RatingsList; 1 util: businessHelpers). `__resetBusinessMap()` para tests. Pendiente: RankingsView.tsx y MapView.tsx (followup)
- **MUI bundle split** (#324): `vite.config.ts` separa `@mui/material` (mui-core, ~450KB) de `@mui/icons-material` (mui-icons, ~34KB). Cache win: cambios en uso de iconos no invalidan el chunk core. `loading="lazy" decoding="async"` aplicado en MenuPhotoSection (con width/height reservados) y MenuPhotoViewer (fullscreen con objectFit)
...
- **Tests**: 253 test files (195 frontend + 57 functions + 1 Firestore rules), 1829 frontend + 528 functions + 16 rules tests (#322 +32 functions; #324 +12 frontend businessMap; #323 +43 frontend offline coverage; #328 +7 frontend helpGroups; #326 +42 frontend ui-ux; #325 +18 functions perf + 7 frontend; #330 +62 frontend hook coverage; #327 +54 frontend admin-metrics: rateLimits/listItems services + RateLimitsSection + AbuseAlerts + FeaturedListsPanel rewire + useAbuseLogsRealtime enabled flag; #338 +47 frontend coverage backfill branches: useBusinessData split + cache-merge race fix, FeedbackForm, SharedListsView, cross-cutting tracker cleanup; #332 +16 rules users.rules.test.ts harness R6/R7/R12 con regression sentinel automatizado). Incluye: utils, services, hooks, contexts, auth, onboarding, follows, activity feed, triggers, aggregates, helpers, readCache, contrast, useRatingPrompt, useCommentEdit, useVerificationCooldown, sharedLists, BusinessScopeContext, businessMap, MapErrorBoundary, helpGroups, forceUpdate utils, fanOut batch, useForceUpdate concurrency+debounce guards, Firestore rules unit tests. Politica: >=80% cobertura para features nuevas. Ver [tests.md](tests.md)
